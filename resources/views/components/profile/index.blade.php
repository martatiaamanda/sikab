      ...
      <div class="row gx-4">
        <div class="col-auto">
          <div class="avatar avatar-xl position-relative">
            <img src="{{$user->user_data->profile_ficture ? asset('storage/profile/'.$user->user_data->profile_ficture) : asset('storage/profile/default.jpg')}}" alt="profile_image" class="w-100 border-radius-lg shadow-sm">
          </div>
        </div>
        ...
        <div class="col-lg-2 col-md-2 my-sm-auto ms-sm-auto me-sm-0 mx-auto mt-3">
          <div class="nav-wrapper d-flex justify-content-end align-items-center">
                <button type="button" class="btn bg-gradient-primary  end-0" data-bs-toggle="modal" data-bs-target="#modalPhoto"><i class="fas fa-edit"></i></button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="col-12 ">
        <div class="card h-100">
          ...
          <div class="card-body p-3">
            <div class="">
                <ul class="list-group">
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">Nama Lenglkap :</strong> &nbsp; {{$user->name}} </li>
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">NIK :</strong> &nbsp; {{$user->NIK}} </li>
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">Alamat :</strong> &nbsp; {{$user->user_data->alamat}} </li>
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">Tempat Tanggal Lahir :</strong> &nbsp; {{$user->user_data->tempat_lahir}} , {{$user->user_data->tanggal_lahir}}</li>
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">Jenis Kelamin :</strong> &nbsp; {{$user->user_data->jenis_kelamin === "L" ? 'laki laki' : 'Perempuan' }} </li>
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">Email :</strong> &nbsp; {{$user->email}} </li>
                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">No Handphone :</strong> &nbsp; {{$user->user_data->no_hp}} </li>
                </ul>
            </div>
          </div>
        </div>
          ...
          <form action={{ route('profile.update', [ 'user' => $user->id])}}  method="POST" enctype="multipart/form-data">
            ...
            <div class="row mb-5">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input name="alamat" id="alamat" type="text" class="form-control" placeholder="Alamat Lengkap" aria-label="alamat" value="{{ $user->user_data->alamat }}">
                    @error('alamat')
                        <p class="text-danger p-0 m-0">{{ $message }}</p>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <input name="tempat_lahir" id="tempat_lahir" type="text" class="form-control" placeholder="Tempat Lahir" aria-label="tempat_lahir" value="{{ $user->user_data->tempat_lahir }}">
                    @error('tempat_lahir')
                        <p class="text-danger p-0 m-0">{{ $message }}</p>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <input name="tanggal_lahir" id="tanggal_lahir" type="date" class="form-control" placeholder="Tanggal Lahir" aria-label="tanggal_lahir" value="{{  $user->user_data->tanggal_lahir }}">
                    @error('tanggal_lahir')
                        <p class="text-danger p-0 m-0">{{ $message }}</p>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select name="jenis_kelamin" id="jenis_kelamin" class="form-control">
                        ...
                        <option value=" L" {{ $user->user_data->jenis_kelamin == 'L' ? 'selected' : '' }}>Laki-laki</option>
                        <option value="P" {{ $user->user_data->jenis_kelamin == 'L' ? 'selected' : '' }}>Perempuan</option>
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input name="no_hp" id="no_hp" type="text" class="form-control" placeholder="Nomor Handphone" aria-label="no_hp" value="{{ $user->user_data->no_hp }}">
                    @error('no_hp')
                        <p class="text-danger p-0 m-0">{{ $message }}</p>
                    @enderror
                </div>
